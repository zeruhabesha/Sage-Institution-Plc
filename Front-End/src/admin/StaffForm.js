...
const [employee,setEmployee]=useState("")
  ...
  const [role,setRole] = useState("staffs");
    ...
    try {
        const res = await axios.post("http://localhost:5000/addacc", user).then(res => {
            if(res.status ===200){
              ...
              // Toastr.danger('This is a danger message', 'Danger');
              // setMessage('');
              ...
              Toastr.error('This is a danger message', 'Danger');
              setMessage('');
            }
          })
        console.log(res);
  ...
  const handleShow = (user) => {
    ...
    setStaffPosition(user.staff_position);
  ...
  const handleRequest1 = ()=>{
   ...
   setEmail(""); setPhone("");  setGender("");  setStaffPosition("");
   setStartDate("");
   setPassword("");
   setRole("");  
    setAddress("")
  ...
  const AddStudent = async (e) => {
    ...
    try {
      ...
      formData.append("staff_position", staff_position);
...
useEffect(() =>{
  ...
  axios.get(`http://localhost:5000/viewstaff`, {
      headers: {
...
fetch(`http://localhost:5000/viewstaff/${_id}`)
...
<table id="dtBasicExample" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
        ...
        <tr key={user._id}>
          ...
          {/* <img src="vendors/images/photo4.jpg" class="border-radius-100 shadow" width="40" height="40" alt=""/> */}
             {/* <img src={user.photo} className="border-radius-100 shadow" width="40" height="40" alt=""/> */}
             {/* <img src={user.photoUrl} class="border-radius-100 shadow" width="40" height="40" alt="" /> */}
             <img src={`http://localhost:5000/uploads/${user.file}`} class="border-radius-100 shadow" width="40" height="40" alt="" />
 ...
 <button class="btn btn-primary"  onClick={() => handleShow(user)}><i className="fas fa-plus"></i></button>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>Add  Staffs </Modal.Title>
        </Modal.Header>
        <Modal.Body>
    ...
    <input type="text" class="form-control" hidden="hidden"id="SID" onChange={(e)=>setSid(e.target.value)} placeholder="SID"/>
  </div>
  <div class="form-group">
  ...
  <div class="form-group">
    <label for="CSchedule">Class-Assigned</label>
    <input type="text" class="form-control" id="CSchedule" onChange={(e)=>setStaffPosition(e.target.value)} placeholder="Enter Class Schedule"/>
  </div> 
  <div class="form-group">
...
>
<Modal.Header closeButton>
  <Modal.Title>Edit Staffs  </Modal.Title>
</Modal.Header>
<Modal.Body>
{/* {hh} */}
...
<div class="pd-ltr-20 xs-pd-20-10">
  ...
  <div class="form-group">
    <label for="staff_position">Class-Schedule</label>
    <input type="text" class="form-control" id="staff_position" defaultValue={staff_position} name="staff_position" onChange={handleChange} placeholder="Enter Class Schedule"/>
  </div>
  <div class="form-group">
    <label for="Start">Start-Date</label>
...
>
<Modal.Header closeButton>
  <Modal.Title>(Active / Inactive)  Staffs {hh} </Modal.Title>
</Modal.Header>
<Modal.Body>
...
<Modal.Footer>
<Button variant="danger" onClick={handleTendMoreClose}>
    Close
  </Button> 
